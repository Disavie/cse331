...
def calculate_branch_sums(node, running_sum, sums):
    ...
    if node.left is None and node.right is None:
        sums.append(running_sum + node.value)
        return

    calculate_branch_sums(node.left,running_sum+node.value,sums)
    calculate_branch_sums(node.right,running_sum+node.value,sums)
